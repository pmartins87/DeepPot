...
$AnalysisCsv = Join-Path $TrainingRoot "analysis\V2_SEL2500_to_V2_SEL3000\task_stability.csv"

if (-not (Test-Path $AnalysisCsv)) {
    throw "SEL3000 stability CSV not found: $AnalysisCsv"
}
...
Write-Host "DeepPot FAST V2 selective continuation: SEL3000 -> SEL3500" -ForegroundColor Green
Write-Host "  selection source: V2_SEL2500 -> V2_SEL3000 per-task stability"
Write-Host "  rule: changed_pct > $ChangedPctThreshold"
Write-Host "  expected selected tasks at 2.0%: 329"
Write-Host "  target for selected tasks: visit_min >= $TargetMinVisits"
Write-Host "  reason: high-sample EV/CI confirmation found 7 persistent greedy mismatches;"
Write-Host "          this is one final cheap CFR pass before final DeepKK-style EV overrides."
...
if ($code -ne 0) { throw "DeepPot SEL3500 continuation exited with code $code" }
