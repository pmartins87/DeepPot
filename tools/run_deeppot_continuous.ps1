...
if ([string]::IsNullOrWhiteSpace($OutDir)) {
    $OutDir = Join-Path $RepoRoot "runs\continuous_master"
}

$logical = [Environment]::ProcessorCount
$workerSource = "explicit"
if ($Workers -le 0) {
    $selected = Join-Path $RepoRoot "runs\worker_benchmark\selected_workers.txt"
    if (Test-Path $selected) {
        $raw = (Get-Content $selected -Raw).Trim()
...
if ($Workers -le 0) {
    ...
    $workerSource = "logical processors minus one fallback"
}
...
Write-Host "DeepPot CONTINUOUS deep training" -ForegroundColor Green
...
if ($code -ne 0) { throw "DeepPot continuous trainer exited with code $code" }
