...
std::array<std::uint8_t, 3> MapFlop(
  ...
  for (std::size_t i = 0; i < 3; ++i) {
    const int rank0 = cards[i].rank - 2;
    out[i] = static_cast<std::uint8_t>(rank0 * 4 + perm[cards[i].suit]);
  }
...
bool PairLess(
    const std::array<std::uint8_t, 3>& flop_a,
    const std::array<std::uint8_t, 2>& hole_a,
    const std::array<std::uint8_t, 3>& flop_b,
    const std::array<std::uint8_t, 2>& hole_b) {
  if (flop_a < flop_b) return true;
  if (flop_b < flop_a) return false;
  ...
  for (int n = 2; n <= 8; ++n) {
    ...
    for (std::size_t slot = 0; slot < flops_.size(); ++slot) {
      ...
      const std::uint64_t bits =
          static_cast<std::uint64_t>(flops_[slot].hole_state_count) * ScenarioCount(n);
      cursor += (bits + 7u) / 8u;
    }
...
bool Strategy::EnsureHoleKeys(std::size_t slot, std::string* error) {
  ...
  std::array<Card, 3> canonical_flop = {{
      DecodeCard(row.card_codes[0]),
      DecodeCard(row.card_codes[1]),
      DecodeCard(row.card_codes[2])}};
  ...
  for (std::size_t i = 0; i < remaining.size(); ++i) {
    for (std::size_t j = i + 1; j < remaining.size(); ++j) {
      ...
      const std::array<std::uint8_t, 2> rep = CanonicalHoleUnderStabilizer(hole, stabilizer);
      keys.push_back(EncodeHoleKey(rep));
    }
  }
...
QueryResult Strategy::Query(
  ...
  if (!loaded_) {
    result.error = "runtime strategy is not loaded";
    return result;
  }
  if (!ValidAndUnique(flop, hole)) {
    result.error = "invalid or duplicate cards";
    return result;
  }

  const int scenario = ScenarioDenseId(num_players, actor_index, prior_stay_mask);
  if (scenario < 0) {
    result.error = "invalid/terminal public scenario";
    return result;
  }
  result.scenario_dense_id = scenario;
  result.global_scenario_code = GlobalScenarioCode(num_players, scenario);
  if (result.global_scenario_code == 0) {
    result.error = "global scenario encoding failed";
    return result;
  }
  ...
  const std::uint32_t flop_key = EncodeFlopKey(canonical_flop);
  const std::unordered_map<std::uint32_t, std::size_t>::const_iterator found =
      slot_by_flop_.find(flop_key);
  if (found == slot_by_flop_.end()) {
    result.error = "canonical flop not present in runtime package";
    return result;
  }
  ...
  if (!EnsureHoleKeys(slot, &hole_error)) {
    result.error = hole_error;
    return result;
  }
  const std::uint16_t target = EncodeHoleKey(canonical_hole);
  const std::vector<std::uint16_t>& keys = hole_key_cache_[slot];
  const std::vector<std::uint16_t>::const_iterator it =
      std::lower_bound(keys.begin(), keys.end(), target);
  if (it == keys.end() || *it != target) {
    result.error = "canonical hole state missing from exact runtime index";
    return result;
  }
  ...
  const std::uint64_t key =
      static_cast<std::uint64_t>(scenario) * row.hole_state_count + hole_id;
  const std::uint64_t byte_index = mode_offsets_[num_players][slot] + (key >> 3);
  if (byte_index >= mode_bits_[num_players].size()) {
    result.error = "strategy bit address out of range";
    return result;
  }
  const std::uint8_t mask = static_cast<std::uint8_t>(1u << (key & 7u));
  result.stay = (mode_bits_[num_players][static_cast<std::size_t>(byte_index)] & mask) != 0;
  result.ok = true;
  result.error.clear();
  return result;
}
