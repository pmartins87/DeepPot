...
from . import continuous_training_fast_v2 as ct


RUNNER_VERSION = "2026-09-08.continuous-fast-v2-runner.1"
    ...
    *,
    ...
    for task in tasks:
        ...
        if (
            ...
            if spath.exists():
                raise RuntimeError(
                    f"existing fast-V2 CFR state has incompatible/missing provenance: {spath}. "
                    "Do not mix mathematical source versions into a canonical master."
                )
            pending.append(task)
    ...
    def request_stop(_signum=None, _frame=None) -> None:
        ...
        if not stop_requested:
            ...
            print(
                "\nDeepPot FAST V2: pause requested. No new chunks will start; active chunks will finish and checkpoint safely.",
                flush=True,
            )
    ...
    print("DeepPot CONTINUOUS FAST V2 exact CFR training")
    print(f"  root: {root}")
    print(f"  kernel: FastChanceSampledCFRV2")
    ...
    print("  EV audit: disabled by design; visit depth + cross-snapshot stability are primary gates")
        ...
        with ProcessPoolExecutor(max_workers=workers, initializer=ct._worker_init) as pool:
            while pending or active:
                if deadline is not None and time.monotonic() >= deadline and not stop_requested:
                    ...
                    print("DeepPot FAST V2: requested wall-time reached; draining active chunks safely.", flush=True)
                ...
                for fut in done:
                    ...
                    try:
                        ...
                        print(f"DeepPot FAST V2 worker error; draining active chunks: {exc}", flush=True)
                        continue
    ...
    print(f"DeepPot FAST V2 continuous stage: {stage.upper()}")
    ...
    if stage == "paused":
        print("  SAFE TO CLOSE. Run the same FAST V2 command later to resume exactly.")
    elif stage == "completed":
        print("  TARGET COMPLETE: every exact infoset reached the configured minimum visit count.")
    elif stage == "error":
        print("  ERROR state recorded in CONTINUOUS_MANIFEST.json; do not resume until fixed.")
...
def main() -> None:
    ap = argparse.ArgumentParser(description="DeepPot fast-V2 interruptible/resumable exact CFR deep training")
